...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        window.statusBarColor = ContextCompat.getColor(this, R.color.flames_dark)
        window.navigationBarColor = ContextCompat.getColor(this, R.color.flames_dark)
        buildShell()
        showDiscover()
    }
    ...
    private fun showDiscover() {
        ...
        val avatar = TextView(this).apply {
            ...
            background = circleDrawable(Color.argb(55, 0, 0, 0))
        }
        cardBox.addView(avatar, LinearLayout.LayoutParams(dp(112), dp(112)).apply { gravity = Gravity.CENTER_HORIZONTAL })
        cardBox.addView(space(24))
        cardBox.addView(label("${p.name}, ${p.age}", 32f, Color.WHITE, true))
        cardBox.addView(label("${p.distance}", 14f, Color.WHITE, false))
        cardBox.addView(space(10))
        cardBox.addView(label(p.bio, 16f, Color.WHITE, false))
        cardBox.addView(space(16))
        val chips = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL }
        p.interests.forEach { interest ->
            val chip = label("  $interest  ", 12f, Color.WHITE, true).apply {
                background = roundedDrawable(Color.argb(50, 0, 0, 0), 50)
                setPadding(dp(8), dp(6), dp(8), dp(6))
            }
            chips.addView(chip, LinearLayout.LayoutParams(-2, -2).apply { marginEnd = dp(7) })
        }
        cardBox.addView(chips)
    ...
    private fun showMatches() {
        content.removeAllViews()
        header("Matches", "People who sparked a connection.")
    ...
    private fun showChats() {
        content.removeAllViews()
        header("Messages", "Keep the spark going.")
        val scroll = ScrollView(this)
        val list = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }
    ...
    private fun showProfile() {
        content.removeAllViews()
        header("My Profile", "Make your profile feel like you.")
        val avatar = TextView(this).apply {
            text = "G"
            textSize = 54f
            gravity = Gravity.CENTER
            setTextColor(Color.WHITE)
            typeface = Typeface.DEFAULT_BOLD
            background = circleDrawable(color(R.color.flames_orange))
        }
    ...
    private fun header(title: String, subtitle: String) {
        ...
        content.addView(label(subtitle, 14f, color(R.color.flames_muted), false))
    ...
    private fun actionButton(text: String, bg: Int, click: () -> Unit): MaterialButton = MaterialButton(this).apply {
        ...
        textSize = 20f
        setTextColor(Color.WHITE)
        backgroundTintList = android.content.res.ColorStateList.valueOf(color(bg))
        cornerRadius = dp(32)
        minWidth = dp(64)
        minimumHeight = dp(56)
        setOnClickListener { click() }
    ...
    private fun matchCard(name: String, subtitle: String, accent: Int): View = MaterialCardView(this).apply {
        ...
        row.addView(button, LinearLayout.LayoutParams(dp(88), dp(48)))
        addView(row)
    ...
    private fun listOption(text: String, click: () -> Unit) {
        val item = TextView(this).apply { this.text = text; textSize = 16f; setTextColor(Color.WHITE); gravity = Gravity.CENTER_VERTICAL; setPadding(dp(18), 0, dp(18), 0); background = roundedDrawable(color(R.color.flames_surface), 18); setOnClickListener { click() } }
